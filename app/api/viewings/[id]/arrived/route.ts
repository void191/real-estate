...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!isValidTransition(viewing.status, ViewingStatus.arrived)) {
      return NextResponse.json(
        { error: `Cannot transition from ${viewing.status} to arrived` },
        { status: 400 }
      );
    }

    if (!isUserAuthorizedForTransition(user!.role, isBuyer, isAgent, viewing.status, ViewingStatus.arrived)) {
      return NextResponse.json(
        { error: 'Forbidden: Not authorized to transition to arrived' },
        { status: 403 }
      );
    }
    ...
    const updated = await prisma.viewing.update({
      where: { id },
      data: { status: ViewingStatus.arrived },
      include: {
        listing: true,
        buyer: { select: { id: true, name: true, email: true, phone: true } },
        agent: { select: { id: true, name: true, avatar_url: true, phone: true, email: true } },
    ...
    // Broadcast update (will signal stream teardown)
    broadcastViewingUpdate(updated);

    return NextResponse.json({ viewing: updated });
  } catch (err) {
    console.error('Arrival transition error:', err);
